...
After this operation, we have a single column of elliptic curve points in $G_1$ originating from the multiplication $\mathbf{L}\mathbf{a}$ and a single column of $G_2$ points from $\mathbf{R}\mathbf{a}$.
